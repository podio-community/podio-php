...
class AppStoreAPI {
  ...
  public function shareApp($app_id, $abstract, $description, $language, $category_ids, $file_ids) {
    if ($response = $this->request('/app_store/', array('app_id' => $app_id, 'abstract' => $abstract, 'description' => $description, 'language' => $language, 'category_ids'=> $category_ids, 'file_ids' => $file_ids ), HTTP_Request2::METHOD_POST)) {
      return json_decode($response->getBody(), TRUE);
    }
  }
}
