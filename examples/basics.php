...
// When possible these calls will return instances of the class in question. You can then work with that object directly. Above we get an instance of the PodioItem class and we can print the item_id and the link to the item. The properties are available directly on the object:
print $item->id;
print $item->link;
...
// For example PodioItem has one 'created_by' (instance of PodioByLine) and it has_many 'comments' (array of PodioComment instances):
print get_class($item->created_by); // Will print 'PodioByLine'

// You can then drill into these relationships easily. E.g. to print the name of the author:
print $item->created_by->name;

// App items are the most complicated part of the Podio API and there are many shortcuts in the PHP library to make your life easier. See items.php for more on items.
